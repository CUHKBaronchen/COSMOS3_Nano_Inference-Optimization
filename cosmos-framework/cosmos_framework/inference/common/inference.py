...
class Inference(ABC):
    ...
    def _record_timer_result(self, func_name: str, seconds: float) -> None:
        """Append an externally measured duration to the benchmark timer."""
        if self._timer is None:
            return
        if self._timer_context:
            context = ".".join(self._timer_context)
            func_name = f"[{context}] {func_name}"
        self._timer.results.setdefault(func_name, []).append(seconds)
