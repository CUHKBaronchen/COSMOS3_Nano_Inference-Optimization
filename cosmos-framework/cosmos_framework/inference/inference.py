...
class SampleDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[SampleArgs, dict[str, Any]]:
        ...
        if sample_args.initial_noise_file is not None:
            noise_data = safetensors.torch.load_file(sample_args.initial_noise_file, device="cuda")
            if set(noise_data) != {"initial_noise"}:
                raise ValueError(
                    f"Initial-noise file must contain only the 'initial_noise' tensor, got {sorted(noise_data)}"
                )
            data_batch["initial_noise"] = noise_data["initial_noise"]
...
class OmniInference(Inference):
    ...
    def _create(cls, setup_args: SetupArgs, **kwargs: Any) -> Self:
        ...
        if setup_args.checkpoint_type == CheckpointType.DCP and setup_args.config_file_type == ConfigFileType.MODULE:
            ...
            if os.environ.get("COSMOS3_SELECTIVE_FP8") == "1":
                from cosmos_framework.inference.quantization import apply_selective_generation_fp8

                apply_selective_generation_fp8(model.net)
    ...
    def generate_batch(
        ...
        if outputs is None:
            ...
            capture_initial_noise = not warmup and any(
                cast(OmniSampleArgs, sample_args).save_initial_noise for sample_args in sample_args_list
            )
            with self._get_timer(f"{self.model.__class__.__name__}.generate_samples_from_batch"):
                outputs = self.model.generate_samples_from_batch(
                    ...
                    cache_diffusion_text=_getattr(sample_args_list, "cache_diffusion_text"),
                    ...
                    record_timing=self._timer is not None,
                    emit_nvtx=self.setup_args.profile,
                    capture_initial_noise=capture_initial_noise,
                )

            timing_breakdown = self.model.last_inference_timing
            for name, stats in timing_breakdown.items():
                self._record_timer_result(
                    f"{self.model.__class__.__name__}.{name}", cast(float, stats["total_seconds"])
                )
            ...
            with sync_distributed_errors():
                for sample_idx, sample_args in enumerate(sample_args_list):
                    if self.should_process_sample(sample_args):
                        ...
                        if timing_breakdown:
                            timing_file = sample_args.output_dir / "timing_breakdown.json"
                            timing_file.write_text(json.dumps(timing_breakdown, indent=2, sort_keys=True))
                            files.append(timing_file)

                        if sample_args.save_initial_noise:
                            initial_noise = self.model.last_initial_noise
                            if initial_noise is None:
                                raise RuntimeError("Initial-noise capture was requested but no noise was captured")
                            noise_file = sample_args.output_dir / "initial_noise.safetensors"
                            safetensors.torch.save_file(
                                {"initial_noise": initial_noise[sample_idx].unsqueeze(0)}, noise_file
                            )
                            files.append(noise_file)
                        ...
                        save_img_or_video(
                            vision_cthw,
                            str(vision_file.with_suffix("")),
                            fps=output_fps,
                            quality=quality,
                            timing_context=lambda stage: self._get_timer(
                                f"{self.model.__class__.__name__}.{stage}"
                            ),
                        )
