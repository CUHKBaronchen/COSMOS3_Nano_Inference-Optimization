...
class SamplingOverrides(OverridesBase):
    ...
    cache_diffusion_text: Training[bool | None] = None
    """Cache timestep-invariant diffusion text states across denoising steps."""

    def _build_sampling(self, model_config: "OmniMoTModelConfig", sample_meta: "SampleMeta"):
        if self.cache_diffusion_text is None:
            self.cache_diffusion_text = False
...
class TransferDataOverrides(OverridesBase, _TransferDataBase):
    ...
    def _validate_transfer_hints(self) -> Self:
        ...
        exempt = hint_field_names | {"control_guidance", "negative_prompt_file"}
        transfer_only = [
            name
