...
def _hf_download(cmd_args: list[str]) -> str:
    ...
    if os.environ.get("HF_HUB_OFFLINE") == "1":
        # Avoid uvx environment resolution in offline runs. The installed CLI
        # still validates the requested revision against the local HF cache.
        cmd = ["hf", "download", "--format=json", *cmd_args]
    else:
        cmd = [
            "uvx",
            f"hf@{HF_VERSION}",
