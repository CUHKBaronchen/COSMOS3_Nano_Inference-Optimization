...
from cosmos_framework.model.vfm.utils.memory import DiffusionTextMemoryValue
...
@pytest.mark.L0
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required by the production attention backend.")
def test_two_way_diffusion_text_cache_matches_full_attention():
    """Cached text K/V must preserve generation attention for multiple packed samples."""
    torch.manual_seed(7)
    device = torch.device("cuda")
    dtype = torch.bfloat16
    sample_lens = [5, 6]
    ...
    und_indexes = torch.tensor([0, 1, 5, 6, 7], device=device, dtype=torch.long)
    gen_indexes = torch.tensor([2, 3, 4, 8, 9, 10], device=device, dtype=torch.long)
    ...
    query = make_pack(torch.randn(sum(sample_lens), num_q_heads, head_dim, device=device, dtype=dtype))
    key = make_pack(torch.randn(sum(sample_lens), num_kv_heads, head_dim, device=device, dtype=dtype))
    value = make_pack(torch.randn(sum(sample_lens), num_kv_heads, head_dim, device=device, dtype=dtype))

    expected = attention.two_way_attention(query, key, value)
    empty_q = get_und_seq(query).new_empty(0, num_q_heads, head_dim)
    empty_kv = get_und_seq(key).new_empty(0, num_kv_heads, head_dim)
    query_gen_only = from_mode_splits(empty_q, get_gen_seq(query), query)
    key_gen_only = from_mode_splits(empty_kv, get_gen_seq(key), key)
    value_gen_only = from_mode_splits(empty_kv, get_gen_seq(value), value)
    memory_value = DiffusionTextMemoryValue(
        und_k=get_und_seq(key).unsqueeze(0),
        und_v=get_und_seq(value).unsqueeze(0),
    )

    actual, _ = attention.two_way_attention_gen_only(
        query_gen_only,
        key_gen_only,
        value_gen_only,
        memory_value,
    )
    torch.testing.assert_close(get_gen_seq(actual), get_gen_seq(expected), atol=0, rtol=0)
    assert get_und_seq(actual).shape == (0, num_q_heads * head_dim)
