...
from cosmos_framework.model.vfm.utils.memory import DiffusionTextMemoryValue, KVToStore, MemoryValue
...
def two_way_attention_gen_only(
...
) -> tuple[FactoredSequencePack, KVToStore]:
    """Run generation attention using cached understanding K/V."""
    assert memory_value.und_k is not None and memory_value.und_v is not None
    assert "causal_seq" in packed_key_states, "Diffusion text cache requires factored sequence packing"

    full_q, full_q_offsets = get_full_only_seq(packed_query_states)
    full_k, _ = get_full_only_seq(packed_key_states)
    full_v, _ = get_full_only_seq(packed_value_states)
    cached_und_k = memory_value.und_k.squeeze(0)
    cached_und_v = memory_value.und_v.squeeze(0)

    all_k = get_all_seq(from_mode_splits(cached_und_k, full_k, packed_key_states))
    all_v = get_all_seq(from_mode_splits(cached_und_v, full_v, packed_value_states))
    full_res = attention(
        full_q.unsqueeze(0),
        all_k.unsqueeze(0),
        all_v.unsqueeze(0),
        cumulative_seqlen_Q=full_q_offsets,
        cumulative_seqlen_KV=packed_query_states["sample_offsets"],
        max_seqlen_Q=packed_query_states["max_full_len"],
        max_seqlen_KV=packed_query_states["max_sample_len"],
    )
    full_out = full_res.squeeze(0).flatten(-2, -1)
    empty_und = full_out.new_empty(0, full_out.shape[-1])
    output = from_mode_splits(empty_und, full_out, packed_query_states)
    kv_to_store = (
        full_k.unsqueeze(0),
        full_v.unsqueeze(0),
        memory_value.und_k,
...
def dispatch_attention(
    ...
    if isinstance(memory_value, DiffusionTextMemoryValue):
        if not isinstance(attention_mask, SplitInfo) or attention_mask.is_three_way:
            raise NotImplementedError("Diffusion text cache only supports two-way attention")
        if memory_value.und_k is not None:
            return two_way_attention_gen_only(
                packed_query_states,
                packed_key_states,
                packed_value_states,
                memory_value,
        ...
        return output, None
    assert memory_value is None, f"Base dispatch_attention does not handle {type(memory_value).__name__}"
