...
from cosmos_framework.model.vfm.utils.memory import DiffusionTextMemoryState, MemoryState
...
class _InferenceTimingCollector:
    """Accumulate CUDA-event timings without synchronizing every sampling step."""

    def __init__(self, *, enabled: bool, emit_nvtx: bool):
        self.enabled = enabled
        self.emit_nvtx = emit_nvtx
        self._events: dict[str, list[tuple[torch.cuda.Event, torch.cuda.Event]]] = collections.defaultdict(list)

    @contextmanager
    def region(self, name: str):
        if self.emit_nvtx:
            torch.cuda.nvtx.range_push(f"cosmos3::{name}")
        start = end = None
        if self.enabled:
    ...
    def finalize(self) -> dict[str, dict[str, float | int]]:
        ...
        for name, events in self._events.items():
            total_seconds = sum(start.elapsed_time(end) for start, end in events) / 1000.0
            results[name] = {
...
class OmniMoTModel(ImaginaireModel):
    ...
    def __init__(self, config: OmniMoTModelConfig):
        ...
        self.last_initial_noise: list[torch.Tensor] | None = None
        self.last_inference_timing: dict[str, dict[str, float | int]] = {}
    ...
    def _prepare_inference_data(
        ...
        initial_noise = resolve_initial_noise(initial_noise, data_batch.get("initial_noise"))
    ...
    def _get_velocity(
        ...
        *,
        ...
        residual_cache: DiffusionResidualCache | None = None,
    ) -> list[torch.Tensor]:
        """
        ...
        Args:
            ...
            memory: Optional persistent per-branch diffusion text cache.
    ...
    def generate_samples_from_batch(
        ...
        capture_initial_noise: bool = False,
        cache_diffusion_text: bool = False,
        **kwargs,
        ...
        Args:
            ...
            cache_diffusion_text (bool): Cache the timestep-invariant understanding
                pathway after its first conditional and unconditional forwards.
        ...
        timing = _InferenceTimingCollector(enabled=record_timing, emit_nvtx=emit_nvtx)
        self.last_initial_noise = None
        self.last_inference_timing = {}

        # Prepare all data (initial noise as list of flattened tensors per sample)
        with timing.region("prepare"):
            (
                sequence_plans,
                gen_data_clean,
                cond_tokens,
        ...
        if capture_initial_noise:
            self.last_initial_noise = [noise.detach().cpu().contiguous() for noise in initial_noise]
        ...
        cond_text_memory = uncond_text_memory = None
        if cache_diffusion_text:
            if self.config.joint_attn_implementation != "two_way":
                raise NotImplementedError("Diffusion text cache requires joint_attn_implementation='two_way'")
            if self.parallel_dims is not None and self.parallel_dims.cp_enabled:
                raise NotImplementedError("Diffusion text cache does not support context parallel inference")
            if no_control_state is not None:
                raise NotImplementedError("Diffusion text cache does not support control-CFG")
            language_model = (net or self.net).language_model
            if not hasattr(language_model, "model") or not hasattr(language_model.model, "layers"):
                raise TypeError(
                    "Diffusion text cache requires language_model.model.layers, "
                    f"got {type(language_model).__name__}"
                )
            num_layers = len(language_model.model.layers)
            cond_text_memory = DiffusionTextMemoryState(num_layers)
            uncond_text_memory = DiffusionTextMemoryState(num_layers)

        cond_residual_cache = None
        if os.getenv("COSMOS3_RESIDUAL_CACHE", "0") == "1":
            if self.parallel_dims is not None and self.parallel_dims.cp_enabled:
                raise NotImplementedError("Residual cache does not support context parallel inference")
            if no_control_state is not None:
                raise NotImplementedError("Residual cache does not support control-CFG")
            if cache_diffusion_text:
                raise NotImplementedError("Residual cache cannot be combined with diffusion text cache")
            raw_reuse_steps = os.getenv("COSMOS3_RESIDUAL_CACHE_STEPS", "")
            if not raw_reuse_steps.strip():
                raise ValueError("COSMOS3_RESIDUAL_CACHE_STEPS is required when residual cache is enabled")
            reuse_steps = frozenset(int(value) for value in raw_reuse_steps.split(",") if value.strip())
            protected_steps = frozenset(
                int(value)
                for value in os.getenv("COSMOS3_RESIDUAL_CACHE_PROTECTED_STEPS", "25").split(",")
                if value.strip()
            )
            cond_residual_cache = DiffusionResidualCache(
                ResidualCacheConfig(
                    num_steps=num_steps,
                    first_layer=int(os.getenv("COSMOS3_RESIDUAL_CACHE_FIRST_LAYER", "8")),
                    last_layer=int(os.getenv("COSMOS3_RESIDUAL_CACHE_LAST_LAYER", "27")),
                    reuse_steps=reuse_steps,
                    input_rel_l1_threshold=float(
                        os.getenv("COSMOS3_RESIDUAL_CACHE_REL_L1_THRESHOLD", "0.05")
                    ),
                    protected_first_steps=int(
                        os.getenv("COSMOS3_RESIDUAL_CACHE_PROTECTED_FIRST_STEPS", "5")
                    ),
                    protected_last_steps=int(
                        os.getenv("COSMOS3_RESIDUAL_CACHE_PROTECTED_LAST_STEPS", "5")
                    ),
                    protected_steps=protected_steps,
                )
            )
            log.info(
                "Enabled conditional residual cache: "
                f"steps={sorted(reuse_steps)}, "
                f"layers={cond_residual_cache.config.first_layer}-"
                f"{cond_residual_cache.config.last_layer}, "
                f"threshold={cond_residual_cache.config.input_rel_l1_threshold}, "
                f"protected_steps={sorted(protected_steps)}"
            )
        ...
        def velocity_fn(noise_x: list[torch.Tensor], timestep: torch.Tensor) -> list[torch.Tensor]:
            ...
            def _single_velocity_fn(tokens: list[list[int]], skip_text_tokens: bool):
                region_name = "uncond_forward" if tokens is uncond_tokens else "cond_forward"
                text_memory = uncond_text_memory if tokens is uncond_tokens else cond_text_memory
                residual_cache = None if tokens is uncond_tokens else cond_residual_cache
                with timing.region(region_name):
            ...
            if _any_needs_control_cfg:
                ...
                with timing.region("cfg_combine"):
        ...
        if scheduler_type == "unipc":
            latents = sampler(
                ...
                shift=shift,
                seed=seed,
                timing_context=timing.region,
            )
            if cond_residual_cache is not None:
                log.info(f"Conditional residual cache summary: {cond_residual_cache.summary()}")
        ...
        self.last_inference_timing = timing.finalize()
