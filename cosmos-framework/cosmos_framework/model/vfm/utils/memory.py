...
@dataclass
class DiffusionTextMemoryValue(MemoryValue):
    """Per-layer cached understanding K/V for diffusion inference."""
...
class DiffusionTextMemoryState(MemoryState):
    """Cache the timestep-invariant understanding pathway across denoising steps."""
    ...
    def init(self, hidden_states: dict, device: torch.device) -> None:
        causal = hidden_states["causal_seq"]
        signature = (int(hidden_states["_num_causal_tokens"]), device, causal.dtype)
        if self._signature is None:
            self._signature = signature
        elif signature != self._signature:
            raise ValueError(
                "Diffusion text cache input changed across denoising steps: "
                f"expected {self._signature}, got {signature}"
            )
        self._replay = all(value is not None for value in self._und_k)

    def read_for_layer(self, layer_idx: int) -> DiffusionTextMemoryValue:
        return DiffusionTextMemoryValue(
    ...
    def write_for_layer(self, layer_idx: int, kv_to_store: KVToStore) -> None:
        ...
        self._und_k[layer_idx] = und_k.detach()
        self._und_v[layer_idx] = und_v.detach()
