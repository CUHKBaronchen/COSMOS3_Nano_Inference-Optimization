...
from typing import IO, Any, Callable, ContextManager, Union
...
def save_img_or_video(
    ...
    quality=None,
    ffmpeg_params=None,
    timing_context: Callable[[str], ContextManager] | None = None,
    **kwargs,
) -> None:
    """
    ...
    def timed(name: str):
        return timing_context(name) if timing_context is not None else nullcontext()
    ...
    if sample.shape[1] == 1:
        ...
        with timed("postprocess"):
            save_obj = PILImage.fromarray(
                rearrange((sample_cpu.float().numpy() * 255), "c 1 h w -> h w c").astype(np.uint8),
                mode="RGB",
            )
        ...
        with timed("transfer"):
            sample_cpu = sample.cpu()
        with timed("postprocess"):
            save_obj = rearrange((sample_cpu.float().numpy() * 255), "c t h w -> t h w c").astype(np.uint8)
